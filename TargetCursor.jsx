// TargetCursor (React Bits) — adapted: global React, gsap from CDN (window.gsap), CSS lives in the DC helmet
const { useEffect, useRef, useCallback, useMemo, useState } = React;
...
const TargetCursor = ({
  ...
  useEffect(() => {
    if (gsapReady) return;
    const t = setInterval(() => {
      if (window.gsap) { setGsapReady(true); clearInterval(t); }
    }, 50);
  ...
  const constants = useMemo(() => ({ borderWidth: 2, cornerSize: 7 }), []);
  ...
  useEffect(() => {
    ...
    const tickerFn = () => {
      ...
      // If the locked target was removed from the DOM (e.g. view switch),
      // release the cursor back to its resting crosshair state.
    ...
    const enterHandler = e => {
      ...
      if (dotRef.current) {
        gsap.to(dotRef.current, { opacity: 0, duration: 0.15, ease: 'power2.out' });
      }
      ...
      const leaveHandler = () => {
        ...
        if (dotRef.current) {
          gsap.to(dotRef.current, { opacity: 1, duration: 0.2, ease: 'power2.out' });
        }
  ...
  useEffect(() => {
    const gsap = window.gsap;
    if (isMobile || !cursorRef.current || !spinTl.current || !gsap) return;
